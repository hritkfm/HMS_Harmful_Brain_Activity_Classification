...
class log_best(object):
    """log best score and return best score"""

    def __init__(self, goal="max"):
        self.goal = goal
        self.initialize()

    def initialize(self):
        self.best_score = -np.inf if self.goal == "max" else np.inf
        self.best_epoch = 0

    def reset(self):
        self.initialize()

    def force_setting(self, value, epoch=None):
        self.best_score = value
        self.best_epoch = epoch

    def update(self, value, epoch=None):
        if self.goal == "max":
