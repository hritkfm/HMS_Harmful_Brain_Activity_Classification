...
class HMSSEDModule(BaseModule):
    ...
    def training_step(self, batch, batch_nb):
        image, target, target_SED = batch["data"], batch["target"], batch["target_SED"]
        ...
        if p < self.p_mix_augmentation:
            ...
            if method == "mixup":
                image, target, target_SED = mixup_data_v2(image, target, target_SED, alpha=2)
            elif method == "cutmix":
                image, target_a, target_b = cutmix_data_v2(image, target, target_SED, alpha=0.4)
            else:
                raise NotImplementedError

        y = self.forward(image)  # (B, n_classes)
        pred = y["pred"]
        k_frame_output = y["kspec_framewise_output"]
        k_spec_output = y["kspec_specwise_output"]

        loss = self.loss(pred, k_frame_output, k_spec_output, target, target_SED)
